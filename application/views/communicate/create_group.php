<!DOCTYPE html>
<html>
	<head>
		<meta charset="utf-8">
        <base href="<?=base_url()?>">
		<meta name="description" content="">
		<meta name="keywords" content="">
		<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">
		<title>小组讨论</title>
		<!-- Set render engine for 360 browser -->
		<meta name="renderer" content="webkit">

		<!-- No Baidu Siteapp-->
		<meta http-equiv="Cache-Control" content="no-siteapp" />
		<!-- Add to homescreen for Chrome on Android -->
		<meta name="mobile-web-app-capable" content="yes">
		<link rel="icon" sizes="192x192" href="assets/i/app-icon72x72@2x.png">
		<!-- Add to homescreen for Safari on iOS -->
		<meta name="apple-mobile-web-app-capable" content="yes">
		<meta name="apple-mobile-web-app-status-bar-style" content="black">
		<meta name="apple-mobile-web-app-title" content="Amaze UI" />

  		<link rel="stylesheet" href="style/small_wechat/assets/css/amazeui.flat.min.css">
 	 	<link rel="stylesheet" href="style/small_wechat/assets/css/amazeui.min.css">
 	 	<link rel="stylesheet" href="style/small_wechat/css/new_one.css">
	</head>
	<body>
		<!--头部-->
		<header data-am-widget="header" class="am-header am-header-default am-header-fixed">

			<h1 class="am-header-title">

				<a href="../main/index.html" class="">创建小组</a>

				<div class="am-header-left am-header-nav">

				<a href="../main/index.html" class="">返回</a>
				</div>
  </h1>
		</header>

<form action="<?php echo 'http://138.128.207.99/education/index.php/communication/small_wechat/create_group_action/'.$user_id.'/'; ?>" method="POST" enctype="UTF-8">
		<div class="show_class op1">
			<label>组名：</label>
			<input type="text" name="group_name" id="group_name" value=""/>
		<div class="butt_1">
				<button type="submit">创建</button>
		</div>
		</div>
</form>

	</body>
</html>
